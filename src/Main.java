...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Introduce the number of the calculator that want print");
        String numberPrintCalculator = scanner.next();

        for (int j = 0; j < user.calculators.size(); j++) {
            if (user.calculators.get(j).name.endsWith(numberPrintCalculator)) {
                user.calculators.get(j).printInfo();
            }
        }
    }
}
